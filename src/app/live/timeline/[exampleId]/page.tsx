...
import { TimelineExampleView } from "@/features/timeline/components/timeline-example-view";
import {
...
/**
 * `/live/timeline/[exampleId]` — one registered example timeline, read-only.
 * The timeline counterpart of `/live/[modelId]`, `/live/sequence/[exampleId]`,
 * `/live/flowchart/[exampleId]`, `/live/usecase/[exampleId]`,
 * `/live/er/[exampleId]`, `/live/dict/[exampleId]` and
 * `/live/gantt/[exampleId]`, and the destination the demo index links its
 * timeline cards at.
 *
 ...
 *
 * WHY THE READABLE WORD HERE AND `tl` NEXT DOOR: nothing is ever minted
 ...
 *
 * ROUTE PRECEDENCE: this nests under the static `timeline` segment, so it can
 * never collide with `/live/[modelId]` — a static segment wins that contest,
 * and it is a different route from the `/live/timeline` alias, which is that
 * segment exactly.
 *
...
export function generateStaticParams(): { exampleId: string }[] {
  return listTimelineExampleIds().map((exampleId) => ({ exampleId }));
}
...
export async function generateMetadata({
  ...
  const example = loadTimelineExample(exampleId);
  if (example.status !== "ok") {
    return { title: "Example not found" };
  }
  const { title, description } = example.file.metadata;
  return {
    title: `${title} — example timeline`,
    description:
      description ??
      `An example milestone timeline: ${title}. Click any event to light it.`,
    alternates: { canonical: `/live/timeline/${exampleId}` },
  };
}

export default async function TimelineExamplePage({
  params,
}: {
  params: Promise<{ exampleId: string }>;
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      ...
      <TimelineExampleView file={example.file} />
    </div>
  );
}
