...
import { UseCaseExampleView } from "@/features/usecase/components/usecase-example-view";
import {
  listUseCaseExampleIds,
  loadUseCaseExample,
} from "@/features/usecase/service/example-service";
import { VIEW_EXAMPLE_PARAM } from "@/features/playground/lib/example-param";

/**
 * `/live/usecase/[exampleId]` — one registered example use-case diagram,
 * read-only. The use-case counterpart of `/live/[modelId]`,
 * `/live/sequence/[exampleId]` and `/live/flowchart/[exampleId]`, and the
 * destination the demo index links its use-case cards at.
 *
 ...
 *
 * WHY THE LONG SEGMENT, when new share links mint against bare `/live` and the
 * seed alias is the short `/live/uc`: nothing is ever minted against THIS
 * path — it is a crawlable page a human or a search engine follows a link to,
 * so the readable word beats the four characters. `/live/uc` stays the
 ...
 *
 * ROUTE PRECEDENCE: this nests under the static `usecase` segment, so it can
...
export function generateStaticParams(): { exampleId: string }[] {
  return listUseCaseExampleIds().map((exampleId) => ({ exampleId }));
}
...
export async function generateMetadata({
  ...
  const example = loadUseCaseExample(exampleId);
  if (example.status !== "ok") {
    return { title: "Example not found" };
  }
  const { title, description } = example.file.metadata;
  return {
    title: `${title} — example use-case diagram`,
    description:
      description ??
      `An example use-case diagram: ${title}. Click any actor or use case to spotlight its relationships.`,
    alternates: { canonical: `/live/usecase/${exampleId}` },
  };
}

export default async function UseCaseExamplePage({
  params,
}: {
  params: Promise<{ exampleId: string }>;
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      ...
      <UseCaseExampleView file={example.file} />
    </div>
  );
}
