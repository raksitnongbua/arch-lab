...
/**
 * `/live/sequence/[exampleId]` — one registered example sequence document,
 * read-only. The sequence-diagram counterpart of `/live/[modelId]`, and the
 ...
 * never collide with `/live/[modelId]` — `sequence` already wins that contest
 * (see `/live/page.tsx`), and these ids live one level deeper again.
 *
...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: { canonical: `/live/sequence/${exampleId}` },
  };
}
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border bg-card px-4 py-2.5 sm:px-6">
        ...
        {/* A plain link, not a document hand-off. This used to compress the
            whole example into a share fragment; `?e=` names it in six
            characters instead, so the URL is short, the playground renders it
            SERVER-SIDE, and the link works with JavaScript disabled — none of
            which a fragment can do, since a fragment never reaches the
            server. */}
        <Link
