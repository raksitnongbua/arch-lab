...
/**
 * `/view/sequence/[exampleId]` — a forwarding alias for the sequence diagram example page,
...
export default async function LegacyViewSequenceExamplePage({
  params,
}: {
  params: Promise<{ exampleId: string }>;
  ...
  const { exampleId } = await params;
  return (
    <AliasForward to={`/live/sequence/${exampleId}`} label="the example" />
  );
}
