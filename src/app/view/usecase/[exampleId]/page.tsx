...
/**
 * `/view/usecase/[exampleId]` — a forwarding alias for the use-case diagram example page,
...
export default async function LegacyViewUseCaseExamplePage({
  params,
}: {
  params: Promise<{ exampleId: string }>;
}): Promise<React.JSX.Element> {
  const { exampleId } = await params;
  return <AliasForward to={`/live/usecase/${exampleId}`} label="the example" />;
}
