...
/**
 * `/view/flowchart/[exampleId]` — a forwarding alias for the flowchart example page,
...
export default async function LegacyViewFlowchartExamplePage({
  params,
}: {
  params: Promise<{ exampleId: string }>;
  ...
  const { exampleId } = await params;
  return (
    <AliasForward to={`/live/flowchart/${exampleId}`} label="the example" />
  );
}
