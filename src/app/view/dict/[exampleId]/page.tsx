...
import { DictExampleView } from "@/features/dict/components/dict-example-view";
import {
  listDictExampleIds,
  loadDictExample,
...
/**
 * `/view/dict/[exampleId]` — one registered example data dictionary, read-only. The
 * ER counterpart of `/view/[modelId]`, `/view/sequence/[exampleId]`,
 * `/view/flowchart/[exampleId]` and `/view/usecase/[exampleId]`, and the
 ...
 *
 * WHY THIS IS NOT THE ALIAS ROUTE, when the seed alias is also `/view/er`:
 * they are different routes — the alias is `/view/er` exactly, this nests one
 * segment deeper, and Next resolves the more specific path first. New share
 * links mint against bare `/view`, so nothing is ever minted against either. nothing is ever minted against THIS
 * path — it is a crawlable page a human or a search engine follows a link to,
 * so the readable word beats the four characters.  * ROUTE PRECEDENCE: this nests under the static `dict` segment, so it can
...
export function generateStaticParams(): { exampleId: string }[] {
  return listDictExampleIds().map((exampleId) => ({ exampleId }));
}
...
export async function generateMetadata({
  ...
  const example = loadDictExample(exampleId);
  if (example.status !== "ok") {
    return { title: "Example not found" };
  }
  const { title, description } = example.file.metadata;
  return {
    title: `${title} — example data dictionary`,
    description:
      description ??
      `An example data dictionary: ${title}. Every field, what it means, and where its value comes from.`,
    alternates: { canonical: `/view/dict/${exampleId}` },
  };
}

export default async function DictExamplePage({
  params,
}: {
  params: Promise<{ exampleId: string }>;
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      ...
      <DictExampleView file={example.file} />
    </div>
  );
}
