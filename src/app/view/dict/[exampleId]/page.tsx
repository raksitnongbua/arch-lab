...
/**
 * `/view/dict/[exampleId]` — a forwarding alias for the data dictionary example page,
...
export default async function LegacyViewDictExamplePage({
  params,
}: {
  params: Promise<{ exampleId: string }>;
}): Promise<React.JSX.Element> {
  const { exampleId } = await params;
  return <AliasForward to={`/live/dict/${exampleId}`} label="the example" />;
}
