...
/*
 * THE CANVAS GROUND USED TO BE CONFIGURED HERE, and deliberately is not any
 * more. Four constants — a field gap, a major step and two stroke weights —
 * described a ground painted IN the drawing's coordinates, by React Flow
 * `<Background>` layers on the C4 hosts and an SVG `<pattern>` on the other
 * seven. That ground scaled with the zoom, which is the defect that removed it.
 *
 * The ground is now the SHEET: one CSS background on the pane that owns the
 * diagram, so its geometry is in device pixels and lives in `globals.css`
 * beside the colours it is drawn in. There is no second language to keep in
 * step, so there is no constant here to keep it in step WITH.
 * `components/ui/diagram-well.tsx` owns the mounting; `check:canvas-grid`
 * measures the result and asserts the pitch does not move with the camera.
 */
