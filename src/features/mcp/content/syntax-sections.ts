...
const SECTION_BUILDERS: Record<SyntaxSectionId, () => SyntaxSection> = {
  ...
  diagrams: () => ({
    ...
    body: [
      ...
      "  `fit` names no direction at all: the diagram is laid out top-down,",
      "  left-to-right, and top-down wrapped into columns, and whichever comes",
      "  out nearest the shape of a screen is the one kept. Prefer it when the",
      "  shape matters more than the reading direction; prefer `tb` or `lr`",
      "  when the flow has a direction a reader is meant to follow.",
