...
function overview(): string {
  return [
    ...
    "  the parser and the serializer (grid positions by ordinal, per-type",
    "  sizes, sentinel timestamps), so terse files still round-trip exactly.",
    "",
...
const SECTION_BUILDERS: Record<SyntaxSectionId, () => SyntaxSection> = {
  ...
  example: () => ({
    ...
    body: [
      ...
      "Geometry is omitted throughout — omitted positions get deterministic",
      "grid defaults, so this file is still lossless.",
      "",
