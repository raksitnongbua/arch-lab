...
the same dash maths and the same 1600ms clock, so the two views light a
connector identically (`pnpm check:sequence-motion` reads the C4 stylesheet and
asserts they still agree). Giving calls a marching dash was tried and dropped,
...
| `pnpm check:sequence-motion`  | Proves idle motion's cross-file facts, which no type can catch: the comet's bands still match the C4 viewer's own dasharrays, offsets and clock (read from its stylesheet, so "same as C4" cannot rot), solid kinds are never given a dasharray (a dashed sync arrow reads as async), the reply's keyframes advance exactly its dash period, the head stays low-duty, and reduced motion removes the comet rather than parking three bright stripes on every line. |
