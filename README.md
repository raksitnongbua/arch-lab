...
**Architecture diagrams as plain text.** Nine notations — C4 models, UML-style
sequence diagrams, flowcharts, use-case diagrams, entity-relationship diagrams,
data dictionaries, gantt charts, milestone timelines and lifecycles — written
...
| Route                                                                                                                                                              | What it is                                                                                                                                                                                                                                                                                                      |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/`                                                                                                                                                                | Landing page, written for someone who has not seen this before: what it is, the two things it does that a drawing tool does not (present, and be written by an agent), and how to start. The hero CTA opens `/live?d=seq`, seeded with a worked flow.                                                           |
...
| `/live/[modelId]`                                                                                                                                                  | Read-only viewer for a registered model (`/live/shopflow`, `/live/order-shop`). Invalid JSON is reported with the validator's JSON-path messages instead of a blank canvas.                                                                                                                                     |
| `/live`                                                                                                                                                            | **The playground.** One pane that takes any supported text — `.alab` in any of the nine notations, arch-lab JSON, or Mermaid — detects it and renders the matching diagram, and the one page where a canvas gesture is available. Also where any `/live#m=…` share link lands.                                  |
| `/live/c4`                                                                                                                                                         | A forwarding alias for `/live`, kept so existing `#m=…` share links and their OG card keep working. `noindex`, canonical `/live`, and absent from the sitemap — a trampoline is not a page.                                                                                                                     |
| `/live/seq`                                                                                                                                                        | The same, seeded: it forwards to `/live?d=seq` with the fragment intact. So do `/live/sequence`, `/live/flow`, `/live/uc`, `/live/er`, `/live/dict`, `/live/gt`, `/live/gantt`, `/live/tl`,                                                                                                                     |
| `/live/timeline`, `/live/lc` and `/live/lifecycle`. **Nothing mints against them** — every Share button and `create_share_link` mints bare `/live`, the real page. |
| `/live/sequence/[exampleId]`                                                                                                                                       | A registered example sequence document, read-only (`/live/sequence/checkout`, `/live/sequence/password-reset`). Statically generated from the example registry.                                                                                                                                                 |
| `/live/gantt/[exampleId]`                                                                                                                                          | A registered example gantt, read-only (`/live/gantt/store-migration`, `/live/gantt/datacentre-move`, `/live/gantt/search-sprint`). Statically generated from the same registry, like every other kind's example route.                                                                                          |
...
| `/mcp`                                                                                                                                                             | How to connect an AI agent, or install the same grammar as a skill. Every tool it documents is read from the same catalogue the server registers from, so the page cannot describe a server that does not exist.                                                                                                |
| `/skill`                                                                                                                                                           | The `.alab` grammar as an Agent Skill — what one install puts in your repository, and what a skill cannot do without the server. Its commands and counts are read from the same catalogue `/mcp` renders.                                                                                                       |
| `/api/mcp`                                                                                                                                                         | The MCP server itself (Streamable HTTP, stateless, unauthenticated, read-only). See `src/features/mcp/README.md`.                                                                                                                                                                                               |
| `/view`, `/view/*`                                                                                                                                                 | Retired: the previous name of this whole family. Every old path is a trampoline forwarding to its `/live` equivalent, carrying the query string and the fragment — a share link's document lives in the fragment, and only a client can carry that across. `noindex`, canonical on `/live`, out of the sitemap. |
| `/editor`                                                                                                                                                          | Retired: a forwarding alias for `/live`, carrying any `#m=` payload across. The C4 canvas on `/live` is editable in place (gated by `CANVAS_EDIT_ENABLED`).                                                                                                                                                     |
