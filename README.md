...
| `/live`                      | **The playground.** One pane that takes any supported text — `.alab` in any of the six notations, arch-lab JSON, or Mermaid — detects it and renders the matching diagram, and the one page where a canvas gesture is available. Also where any `/live#m=…` share link lands.                                   |
...
| `/live/seq`                  | The same, seeded: it forwards to `/live?d=seq` with the fragment intact. So do `/live/sequence`, `/live/flow`, `/live/uc`, `/live/er` and `/live/dict`. **Nothing mints against them** — every Share button and `create_share_link` mints bare `/live`, the real page.                                          |
