...
stroke laid over it. Calls are solid, and stay solid: a light runs along them
instead, brightening each stop of the line's own gradient in turn. Giving them
a marching dash was tried and dropped, because a dashed sync arrow reads as an
async one — the motion was overwriting the message kind. Each line is also
...
| `pnpm check:sequence-motion`  | Proves idle motion's cross-file facts, which no type can catch: solid kinds are never given a dasharray (a dashed sync arrow reads as async), the reply's keyframes advance exactly its dash period, the light's stagger divides by the stop count the renderer emits, its gradient is nested where the CSS selector can actually reach it, reduced motion parks each kind on its meaningful appearance, and no overlay path survives. |
